...
func ParseArgs(args []string) (*Options, error) {
	...
	fs.StringVar(&opts.Env, "env", "local", "指定使用的环境名称（默认：local）")
	...
	// 处理无横线参数: <http file> [request name]
	if len(remaining) > 0 {
		opts.HTTPFile = remaining[0]
		
		// 如果有第二个参数且没有通过-request指定请求名称，则使用第二个参数作为请求名称
		if len(remaining) > 1 && opts.RequestName == "" {
			opts.RequestName = remaining[1]
		}
	}
...
func PrintUsage(w io.Writer, progName string) {
	...
	fmt.Fprintf(w, "  -env <n>          指定使用的环境名称（默认: local）\n")
